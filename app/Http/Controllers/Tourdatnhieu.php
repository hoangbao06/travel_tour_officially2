...
class Tourdatnhieu extends Controller
{
    public function TourDatNhieu()
    {
        ...
        $TourDatNhieu = DB::select(DB::raw(" SELECT tour.nameTour,COUNT(hoadontour.idHoaDon) as SoLuong_HDCT FROM `hoadontour` INNER JOIN tour ON tour.idTour = hoadontour.idTour WHERE hoadontour.trangThai = 1 GROUP BY (tour.nameTour) ORDER BY SoLuong_HDCT DESC "));
