...
class thongkedoanhthu extends Controller
{
    public function ThongKeSLnam()
    {
        ...
        $thongKeNam = HoaDonTour::whereyear('ngayDatTour',  '2022')->where('trangthai', '=', '1')->sum('tongTien');

        $thongKeThang = DB::select(DB::raw(" SELECT month(ngayDatTour) AS thongkethang,SUM(tongTien)
        as tongTien FROM `hoadontour` WHERE trangthai = 1 AND year(ngayDatTour) = 2022 GROUP BY (thongkethang)"));
